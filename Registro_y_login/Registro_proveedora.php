...
if(isset($_POST['enviar'])) {
    $Rol = isset($_POST['Rol']) ? $_POST['Rol'] : 3; //Rol 3 Es para mujeres que puden subir sus productos, subir historias de exito, comentar, comprar, etc.
    $Nombre = $_POST['Nombre'];
    $Correo = $_POST['Correo'];
    ...
    if ($Comprobacion == 0) {
        ...
        if(mysqli_query($Conexion, $Insertarcuenta)) {
            echo "<script> alert('Te has registrado con el usuario: $Usuario , gracias por registrarse');window.location= '../Views/Cuenta.php' </script>";
        } else {
            echo "<script> alert('Hubo un error en el registro. Por favor, inténtelo de nuevo.'); window.location= '../Views/Cuenta.php'</script>";
        }
    } else {
        echo "<script> alert('No puedes registrar a este usuario: $Usuario porque ya esta registrado. Por favor, intenta con otro usuario.'); window.location= '../Views/Cuenta.php'</script>";
    }
}
