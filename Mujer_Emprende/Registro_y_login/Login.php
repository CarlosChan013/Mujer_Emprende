    ...
    if($result) {
        if(mysqli_num_rows($result) > 0) {
            ...
            if(password_verify($Password, $stored_password)) {
                ...
                echo "<script> alert('Contraseña incorrecta. Por favor, inténtelo de nuevo.'); window.location= '../Views/Cuenta.html'</script>";
            }
        } else {
            echo "<script> alert('Usuario no encontrado. Por favor, registre una cuenta.'); window.location= '../Cuenta/Registro.php'</script>";
        }
    } else {
